...
import {
  ...
  Package, MapPin, User, Calendar, CheckCircle, Clock, AlertCircle, Truck,
...
import { getPipelines, getDispatchWorkers } from '../../../api/endpoints/pipeline.api'
import AssignWorkerControl from '../../pipeline/components/AssignWorkerControl'
...
export default function ChallansListPage() {
  ...
  const fetchChallansList = useCallback(async () => {
    setLoading(true)
    try {
      const [res, pipeRes] = await Promise.all([getChallans(), getPipelines()])
      if (res.success) {
        setChallans(res.data)
      } else {
        toast.error(res.error || 'Failed to fetch challans')
      }
      // Pipeline rows carry the real dispatch worker + timeline (role-scoped server-side).
      if (pipeRes.success) {
        const map = {}
        pipeRes.data.forEach(p => { map[p.order_id] = p })
        setPipelineMap(map)
      }
      if (isIM) {
        const workersRes = await getDispatchWorkers()
  ...
  const handleAssigned = useCallback((updated) => {
    if (updated?.order_id) setPipelineMap(m => ({ ...m, [updated.order_id]: updated }))
  }, [])
  ...
  const formattedChallans = useMemo(() => {
    return challans.map(c => {
      ...
      // Pull the REAL dispatch worker + timeline from the order's pipeline row.
      const pipe = pipelineMap[order.id] || null
      const assignedWorker = pipe?.dispatchWorker?.name || null
      ...
      return {
        ...
        requested_by: salesManager.name || 'N/A',           // Sales Manager (who placed it)
        dispatched_by: assignedWorker || 'Unassigned',       // Dispatch Worker (who delivers it)
        worker_assigned: !!assignedWorker,
        approved_at: pipe?.admin_approved_at || null,
        assigned_at: pipe?.dw_assigned_at || null,
        eta: pipe?.expected_delivery_at || null,
        delivered_at: pipe?.delivered_at || null,
        pipeline_stage: pipe?.stage || null,
        items: mappedItems,
        total_items: items.length,
        total_qty: totalQty,
        status,
        vehicle_no: pipe?.vehicle_number || '—',
        driver: pipe?.driver_name || '—',
      }
    })
  }, [challans, pipelineMap])
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-surface-200 dark:border-surface-700 bg-surface-50/70 dark:bg-surface-800/70 text-xs font-semibold text-surface-600 dark:text-surface-400 uppercase tracking-wider">
                    ...
                    <th className="px-5 py-3.5">Dispatch Worker</th>
                    ...
                    <tr key={c.id} className="table-row-hover">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5 text-xs text-surface-600 dark:text-surface-400">
                          <User className="h-3.5 w-3.5 text-surface-400" /> {c.requested_by}
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        {c.worker_assigned ? (
                          <div className="text-xs">
                            <div className="flex items-center gap-1.5 font-medium text-surface-800 dark:text-surface-200">
                              <Truck className="h-3.5 w-3.5 text-primary-500" /> {c.dispatched_by}
                            </div>
                            {c.eta && (
                              <div className="mt-0.5 flex items-center gap-1 text-surface-400">
                                <Clock className="h-3 w-3" /> by {new Date(c.eta).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                              </div>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          {isIM && (
                            <AssignWorkerControl
                              orderId={c.order_id}
                              pipeline={pipelineMap[c.order_id]}
                              workers={workers}
        ...
        {viewChallan && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="rounded-xl bg-surface-50 dark:bg-surface-700/40 border border-surface-200 dark:border-surface-700 p-4 space-y-2">
                <p className="text-xs font-semibold uppercase tracking-wider text-surface-400">Delivery</p>
                <p className="text-sm text-surface-700 dark:text-surface-300">
                  <span className="text-surface-400 text-xs">Requested by: </span>{viewChallan.requested_by}
                </p>
                <p className="text-sm text-surface-700 dark:text-surface-300 flex items-center gap-1">
                  ...
                  <span className="text-surface-400 text-xs">Dispatch worker: </span>
                  {viewChallan.worker_assigned ? viewChallan.dispatched_by : <span className="text-surface-400">Unassigned</span>}
                </p>
                {viewChallan.eta && (
                  <p className="text-sm text-surface-700 dark:text-surface-300">
                    <span className="text-surface-400 text-xs">To be delivered by: </span>
                    {new Date(viewChallan.eta).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}
                  </p>
                )}
                {viewChallan.vehicle_no && viewChallan.vehicle_no !== '—' && (
                  <p className="text-sm text-surface-700 dark:text-surface-300">
                    <span className="text-surface-400 text-xs">Vehicle: </span>{viewChallan.vehicle_no}
                    {viewChallan.driver && viewChallan.driver !== '—' && <span className="text-surface-400"> · {viewChallan.driver}</span>}
                  </p>
                )}
              </div>
            </div>

            {/* Delivery timeline (Dunzo-style) */}
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-surface-400 mb-2">Timeline</p>
              <div className="rounded-xl border border-surface-200 dark:border-surface-700 divide-y divide-surface-100 dark:divide-surface-700 text-sm">
                {[
                  { label: 'Order placed', by: viewChallan.requested_by, at: viewChallan.date },
                  { label: 'Approved & assigned', by: viewChallan.dispatched_by !== 'Unassigned' ? viewChallan.dispatched_by : null, at: viewChallan.assigned_at || viewChallan.approved_at },
                  { label: 'Expected delivery', by: null, at: viewChallan.eta, pending: !viewChallan.delivered_at },
                  { label: 'Delivered', by: null, at: viewChallan.delivered_at, hideIfEmpty: true },
                ].filter(step => !(step.hideIfEmpty && !step.at)).map((step, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-2.5">
                    <span className="text-surface-600 dark:text-surface-300">{step.label}{step.by ? <span className="text-surface-400"> · {step.by}</span> : ''}</span>
                    <span className={cn('text-xs', step.pending ? 'text-warning-600 dark:text-warning-400' : 'text-surface-500')}>
                      {step.at ? new Date(step.at).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—'}
                    </span>
                  </div>
                ))}
              </div>
