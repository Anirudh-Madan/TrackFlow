...
export default function OrdersListPage() {
  ...
  const [pipelineMap, setPipelineMap] = useState({}) // order_id -> pipeline row

  const fetchOrdersList = useCallback(async () => {
    setLoading(true)
    try {
      // IM now needs to see approved orders too (to assign workers), so fetch all
      // and load the pipeline + worker list alongside for the inline control.
      if (isIM) {
        const [ordersRes, pipeRes, workersRes] = await Promise.all([
          getOrders(), getPipelines(), getDispatchWorkers(),
        ])
  ...
  // Update a single pipeline entry after an assignment without a full refetch.
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="rounded-xl bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 p-4 space-y-2">
                ...
                <p className="text-sm font-semibold text-surface-800 dark:text-surface-100">Requested by: {viewOrder.salesManager?.name}</p>
                <p className="text-xs text-surface-500">Submitted: {new Date(viewOrder.created_at || new Date()).toLocaleString('en-IN')}</p>
              </div>
            </div>

            {/* Dispatch worker assignment (IM) */}
            {isIM && (
              <div className="rounded-xl border border-primary-200 bg-primary-50/50 p-4 dark:border-primary-900/40 dark:bg-primary-900/10">
                <p className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-primary-600 dark:text-primary-400">
                  <Truck className="h-3.5 w-3.5" /> Assign Dispatch Worker
                </p>
                {(() => {
                  const pipe = pipelineMap[viewOrder.id]
                  const dw = pipe?.dispatchWorker?.name
                  if (dw && !['IM_APPROVAL', 'DW_ASSIGNMENT'].includes(pipe?.stage)) {
                    return <p className="text-sm text-surface-700 dark:text-surface-300">Assigned to <span className="font-semibold">{dw}</span>{pipe?.expected_delivery_at ? ` · ETA ${new Date(pipe.expected_delivery_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}` : ''}</p>
                  }
                  return (
                    <div className="space-y-2">
                      <AssignWorkerControl
                        ...
                        pipeline={pipe}
                        workers={workers}
                        onAssigned={(updated) => { handleAssigned(updated); }}
                      />
                      {!pipe && <p className="text-xs text-surface-400">Pick a worker to approve this order and assign delivery — no separate Admin approval needed.</p>}
                      {dw && <p className="text-xs text-surface-500">Currently: {dw} — you can reassign above.</p>}
                    </div>
                  )
                })()}
              </div>
